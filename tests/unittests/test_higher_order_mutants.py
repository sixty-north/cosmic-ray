...
def test_multiple_mutations_generated_correctly(monkeypatch):
    ...
    mock_mutations = [MutationSpec(Path(f"path{i}"), f"operator{i}", i, (i, 0), (i, 1)) for i in range(5)]
...
def test_mutation_limit_first_order(monkeypatch):
    ...
    mock_mutations = [MutationSpec(Path(f"path{i}"), f"operator{i}", i, (i, 0), (i, 1)) for i in range(10)]
    ...
    with (
        mock.patch("cosmic_ray.commands.init._generate_mutations", return_value=mock_mutations),
...
def test_disable_overlapping_mutations():
    ...
    with mock.patch("cosmic_ray.commands.init._generate_mutations", return_value=mock_mutations):
        ...
        work_items_with_overlap = list(_all_work_items([], {}, mutation_order=2, disable_overlapping=False))

        # Test with overlapping mutations disabled (default)
        work_items_no_overlap = list(_all_work_items([], {}, mutation_order=2, disable_overlapping=True))
        ...
        for item in work_items_with_overlap:
            if len(item.mutations) == 2:
                ...
                if (
                    m1.module_path == Path("path4")
                    and m2.module_path == Path("path4")
                    and m1.start_pos == (5, 0)
                    and m2.start_pos == (5, 0)
                ):
