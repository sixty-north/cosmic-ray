...
def test_jobs_with_results_are_not_pending(work_db):
    work_db.add_work_item(WorkItem("path", "operator", 0, (0, 0), (0, 1), "job_id"))
    work_db.set_result(
        "job_id",
...
def test_num_work_items(work_db):
    ...
    for idx in range(count):
        work_db.add_work_item(WorkItem("path", "operator", 0, (0, 0), (0, 1), "job_id_{}".format(idx)))
...
def test_clear_work_items_removes_results(work_db):
    for idx in range(10):
        work_db.add_work_item(WorkItem("path", "operator", 0, (0, 0), (0, 1), "job_id_{}".format(idx)))
        work_db.set_result("job_id_{}".format(idx), WorkResult(WorkerOutcome.NORMAL))
